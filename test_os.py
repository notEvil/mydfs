...
@hys.composite
def _choose(draw, n, withATime=False, withMTime=False, withUser=False, withGroup=False, withMode=False):
    directory = draw(
        hypothesis_fs.directories(
            withATime=withATime,
            withMTime=withMTime, withUser=withUser, withGroup=withGroup, withMode=withMode).filter(
                lambda directory: len(directory.ExistingFiles) != 0 or len(directory.NotExistingFiles) != 0))
...
@hy.given(_choose(1, withATime=True, withMTime=True, withUser=True, withGroup=True, withMode=True))
def test_stat(args):
    directory, ((path, file, exists), ) = args
...
@hy.given(_choose(1, withUser=True, withGroup=True, withMode=True), _mode)
...
@hy.given(_choose(1, withATime=True, withMTime=True), hypothesis_fs.times, hypothesis_fs.times)
...
@hy.given(
    _choose(1, withUser=True, withGroup=True),
...
@hy.given(_choose(1, withMode=True), hypothesis_fs.modes)
